...
def stroke_thickness(image): 
    ...
    image_edt = ndimage.distance_transform_edt(image)
    image_edt *= 10
...
def average_thickness(image_list):
    average = []
    for im in image_list:
        average.append(stroke_thickness(im))
...
def load_data_wrapper():
    ...
    #tr_d = (x,y) = ([[784px], [784px]... [784px]], [digit, digit... digit])
    '''for i in range(len(tr_d[0])):
        image_edt = ndimage.distance_transform_edt(tr_d[0][i])
        dist_values = [i for i in image_edt]
        thick_point = np.amax(dist_values)
        image_edt = np.float32(image_edt)/thick_point
        tr_d[0][i] = image_edt'''
